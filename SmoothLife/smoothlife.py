...
def show_animation():
    ...
    animation.FuncAnimation(fig, animate, interval=60, blit=True)
    plt.show()


def save_animation(path, count, intensity, style):
    if count:
        count = int(count)
    ...
    # h = 1080
    sl = SmoothLife(h, w)
    sl.add_speckles(count, intensity)

    # Matplotlib shoves a horrible border on animation saves.
    # We'll do it manually. Ugh
    fps = 10
    frames = 300
    w = FFmpegWriter(path, inputdict={"-r": str(fps)})
    for i in range(frames):
        frame = getattr(cm, style)(sl.field)
        frame *= 255
